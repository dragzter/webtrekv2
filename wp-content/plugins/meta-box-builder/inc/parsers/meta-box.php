<?php
class MBB_Parser_Meta_Box extends MBB_Parser_Base {
	...
	private function parse_location() {
		...
		if ( in_array( $for, array( 'post_types', 'taxonomies' ), true ) && isset( $this->{$for} ) ) {
			$this->{$for} = wp_list_pluck( $this->{$for}, 'slug' );
		}
		if ( 'settings_pages' === $for && isset( $this->{$for} ) ) {
			$this->{$for} = wp_list_pluck( $this->{$for}, 'id' );
		}
	...
	private function parse_field( &$field ) {
		$parser = MBB_Parser_Field::from_array( $field );
		$parser->parse();
