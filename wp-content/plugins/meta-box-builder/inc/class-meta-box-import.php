...
class Meta_Box_Import {
	...
	public static function submenu() {
		add_submenu_page( 'meta-box', __( 'Import', 'meta-box-builder' ), __( 'Import', 'meta-box-builder' ), 'manage_options', 'meta-box-import', array( __CLASS__, 'page' ) );
	}
	...
	public static function page() {
		?>
		<div class="wrap">
			<h1><?php esc_html_e( 'Import Field Groups', 'meta-box-builder' ); ?></h2>
			<?php if ( isset( $_GET['success'] ) ) : ?>
				<div class="updated">
					<p><?php esc_html_e( 'Tigh tigh tigh! Field groups has imported successfully!', 'meta-box-builder' ); ?></p>
				</div>
			<?php endif; ?>
			...
			<form enctype="multipart/form-data" id="import-upload-form" method="post" class="wp-upload-form" action="edit.php?post_type=meta-box&amp;page=meta-box-import">
				<p>
					<label for="upload"><?php esc_html_e( 'Choose a file from your computer:', 'meta-box-builder' ); ?></label>
	...
	public static function handle_post() {
		...
		if ( ! empty( $_POST ) && ! empty( $_FILES ) && isset( $_GET['post_type'] ) && 'meta-box' === $_GET['post_type'] && ends_with( $_FILES['import']['name'], '.dat' ) ) {
			...
			if ( wp_verify_nonce( $uploadnonce, 'uploadnonce' ) ) {
				...
				foreach ( $meta_boxes as $meta_box ) {
					...
					$post_excerpt = $meta_box_content['post_excerpt'];
					$post_excerpt = addslashes( $post_excerpt );
					$meta_box_content['post_excerpt'] = $post_excerpt;
				...
				wp_redirect( admin_url( '/admin.php?page=meta-box-import&success' ) );
				die;
			}
		}
	}
